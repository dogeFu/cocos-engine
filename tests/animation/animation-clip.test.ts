import { Color, js, lerp, Size, Vec2, Vec3, Vec4 } from "../../cocos/core";
import {
    AnimationClip,
    AnimationState,
    AnimationManager,
} from "../../cocos/animation";
import {
    ColorTrack,
    ComponentPath,
    HierarchyPath,
    ...
    Track,
    VectorTrack,
} from "../../cocos/animation/animation";
...
test("Common targets that modify eulerAngles", () => {
    ...
    const mockInstance = jest
        .spyOn((global as any).cc.director, "getAnimationManager")
        .mockImplementation(() => {
...
describe("Custom track setter", () => {
    class Target {
        public setValue(value: Target["_value"]) {
...
test("animation state", () => {
    ...
    clip.duration = 2.0;
    const keys = [0, 0.1, 0.2];
    const values = [new Vec3(), new Vec3(1), new Vec3(2)];
    clip.keys = [keys];
    clip.curves = [
        {
            modifiers: ["position"],
...
test("Warn on track binding failure", () => {
    ...
    const [lastId, ...lastArgs] =
        warnIDWatcher.captured[warnIDWatcher.captured.length - 1];
    expect(lastId).toBe(3937);
    expect(lastArgs).toStrictEqual([clip.name, node.name]);
    warnIDWatcher.stop();
});
...
describe(`Component-wise animation`, () => {
    ...
    const newVecXTrack = (x: 2 | 3 | 4) => {
        const track = new VectorTrack();
        track.componentsCount = x;
        return track;
    };
    ...
    function check(evaluate: EvaluateClipSingleFrame, specs: typeof SPECS) {
        ...
        const expects = specs.map(
            ({
                ...
                [...defaultKeys, ...animatedKeys]
                    .sort()
                    .forEach(([propertyKey, _], keyIndex, keys) => {
                        const propertyValue = gen(
                            keyIndex,
                            keys.length,
                            0.1,
                            0.5,
                        ...
                        defaultProperties[propertyKey] =
                            defaultValue[propertyKey]; // Don't use `= propertyValue` here since for example, color matters
                    });

                // Register the value to the dummy component so animation system can animate it.
                if (constructor === Vec3) {
                    // Specially handle `Vec3`
                    node.position = (defaultValue as Vec3).clone();
                } else {
                    Object.defineProperty(component, constructor.name, {
                ...
                if (constructor === Vec3) {
                    ...
                    track.path
                        .toComponent(DummyComponent)
                        .toProperty(constructor.name);
                }
                const expectedAnimatedProperties: Record<string, number> = {};
                [...animatedKeys].forEach(
                    ([propertyKey, channelIndex], keyIndex, keys) => {
                        const propertyAnimationValue = gen(
                            keyIndex,
                            keys.length,
                            0.5,
                            1.0,
                        );
                        [...track.channels()][channelIndex].curve.addKeyFrame(
                            0.0,
                            propertyAnimationValue,
                        );
                        const o = new constructor();
                        o[propertyKey] = propertyAnimationValue;
                        expectedAnimatedProperties[propertyKey] =
                            o[propertyKey];
                    },
                );
                clip.addTrack(track);
        ...
        // Check test data.
        specs.forEach(
            ({ constructor, defaultKeys, animatedKeys }, specIndex) => {
                const { defaultProperties, expectedAnimatedProperties } =
                    expects[specIndex];
                const object =
                    constructor === Vec3
                        ? node.position
...
type EvaluateClipSingleFrame = (
    clip: AnimationClip,
    node: Node,
    time: number,
) => void;

function evaluateClipSingleFrameTraditional(
    clip: AnimationClip,
    node: Node,
    time: number,
) {
