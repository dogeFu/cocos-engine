    ...
    mixin(
        {
                ...
                cc: {
                    animation: {
                        ...
                        TCStateMotionTimeBinding: {
                            menu: "动画流逝时间绑定",
                            tooltip:
                                "使用状态中动画的（标准化的）流逝时间作为条件值。",
                            abbr: {
                                label: "动画流逝时间",
                                tooltip:
                                    "过渡源状态中，动画的（标准化的）流逝时间。",
                            },
                        },
                    },
                },
            ...
            skybox: {
                EnvironmentLightingType:
                    "选择三种环境光照类型：<br>手调半球漫反射，无环境反射；<br>自动生成半球漫反射，带环境反射；<br>更精确的漫反射图，带环境反射；",
            ...
            fog: {
                enabled: "雾开关",
                accurate:
                    "切换顶点雾和像素雾。勾选该项则使用像素雾，像素雾在顶点数少面积大的物体上有更精确的雾化效果；顶点雾则有更好的性能。",
                fogColor: "内散射颜色",
            ...
            shadow: {
                ...
                shadowColor: "平面阴影颜色",
                planeDirection:
                    "阴影接收平面的法线，垂直于阴影，用于调整阴影的倾斜度",
                planeHeight: "阴影接收平面距离原点的高度",
                planeBias: "平面阴影计算中的位置偏移值",
                shadowMapSize:
                    "阴影贴图分辨率，目前支持 Low_256x256、Medium_512x512、High_1024x1024、Ultra_2048x2048 四种精度的纹理",
            ...
            camera: {
                ...
                clear_flags:
                    "相机的缓冲清除标志位，指定帧缓冲的哪部分要每帧清除",
                color: "相机的颜色缓冲默认值",
                depth: "相机的深度缓冲默认值",
                stencil: "相机的模板缓冲默认值",
                projection: "相机的投影类型",
                fov_axis:
                    "指定视角的固定轴向，在此轴上不会跟随屏幕长宽比例变化",
                fov: "相机的视角大小",
            ...
            lights: {
                ...
                luminance: "光亮度",
                visibility:
                    "可见性掩码，声明在当前精确光源中可见的节点层级集合（对方向光不生效）",
                term: "当前使用的光度学计量单位",
                size: "光源大小",
                range: "光源范围",
                ...
                shadowPcf:
                    "开启软阴影，目前支持 HARD（硬采样）、SOFT（4 倍采样）、SOFT_2X（9 倍采样）、SOFT_4X（16 倍采样）类型",
                shadowBias:
                    "增加深度偏移值（世界空间单位）可以有效消除阴影摩尔纹，但是过大的值可能造成漏光现象",
                shadowNormalBias:
                    "法线深度偏移值（世界空间单位），可以消除物体表面朝向平行于阳光方向的阴影摩尔纹，<br>防止曲面出现锯齿状；但是过大的值可能会造成阴影位置偏差",
                shadowSaturation:
                    "阴影饱和度，建议设置为 1.0。<br>若需要减小方向光阴影的饱和程度，推荐通过增加环境光来实现，而不是调节该值。",
                shadowDistance:
                    "阴影有效距离（世界空间单位），该值越大则阴影精度越低",
                shadowInvisibleOcclusionRange:
                    "如果有背后的潜在投射物阴影丢失，请增大该值（世界空间单位）",
                enableCSM: "开启 CSM 模式",
                shadowFixedArea:
                    "切换固定区域和 CSM 模式。固定区域是一种旧模式，我们并不推荐使用。<br>勾选该项则开启 CSM 模式，该模式下阴影会跟随方向光节点的位置，在方向光包围盒附近分布，而非跟随相机。",
                shadowNear: "固定区域开始值",
                shadowFar: "固定区域结束值",
                shadowOrthoSize: "固定区域大小，该值越大则阴影精度越低",
                shadowAdvancedOptions: "阴影高级选项",
                csmLayersTransition:
                    "是否开启级联阴影层级过渡（提升质量，降低性能）",
                csmTransitionRange:
                    "级联阴影层级过渡范围(NDC空间: 取值范围为 0 ~ 1)",
            },
            sprite: {
                original_size:
                    "是否使用图片资源的原始尺寸作为 Sprite 节点的 size",
                edit_button: "编辑",
                select_button: "选择",
                select_tooltip: "选择 Atlas 中的其他 SpriteFrame",
                edit_tooltip: "打开 Sprite 编辑器，设置九宫格等数据",
                fill_type:
                    "填充方向，可以选择横向 Horizontal，纵向 Vertical 和扇形 Radial 三种方向",
                fill_center: "扇形填充时，指定扇形的中心点，取值范围 0 ~ 1",
                fill_start:
                    "填充起始位置，输入一个 0 ~ 1 之间的小数表示起始位置的百分比",
            ...
            button: {
                click_event: {
                    ...
                    handler: "处理点击事件的方法",
                    customEventData:
                        "传给事件处理函数的额外参数，这个数据会当作最后一个参数传递给事件处理函数。",
                },
                interactable:
                    "按钮是否可交互，这一项未选中时，按钮处在禁用状态",
                ...
                click_events:
                    "按钮点击事件的列表。先将数量改为1或更多，就可以为每个点击事件设置接受者和处理方法",
            },
            canvas: {
                camera: "2D 渲染相机",
                ...
                fit_height:
                    "自动缩放 Canvas 使设计分辨率的高度充满设备屏幕的高度",
                fit_width:
                    "自动缩放 Canvas 使设计分辨率的宽度充满设备屏幕的宽度",
            },
            forceOvertimeModule: {
                x: "X 轴方向上的加速度分量",
                y: "Y 轴方向上的加速度分量",
            ...
            scrollbar: {
                ...
                auto_hide_time:
                    "没有滚动动作后经过多久会自动隐藏<br>注意：只有当  Enable Auto Hide 为 true 时才有效",
            },
            scrollview: {
            ...
            pageview: {
                ...
                pageTurningEventTiming:
                    "设置 Page View 页面自动滚动动画结束的阈值，<br>修改此值可以调整 Page View 事件的发送时机。",
                indicator: "页面视图指示器组件",
            ...
            particleSystemRenderer: {
                renderMode: "设定粒子生成模式",
                velocityScale:
                    "在粒子生成方式为 Stretched Billboard 时,对粒子在运动方向上按速度大小进行拉伸",
                lengthScale:
                    "在粒子生成方式为 Stretched Billboard 时,对粒子在运动方向上按粒子大小进行拉伸",
                mesh: "粒子发射的模型",
            ...
            textureAnimationModule: {
                ...
                randomRow:
                    "随机从动画贴图中选择一行以生成动画,<br>此选项仅在动画播放方式为 Single Row 时生效",
                rowIndex:
                    "从动画贴图中选择特定行以生成动画,<br>此选项仅在动画播放方式为 Single Row 时且禁用 Random Row 时可用",
            },
            toggle: {
                interactable:
                    "Toggle 是否可交互，这一项未选中时，Toggle 处在禁用状态",
                transition: "Toggle 状态变化时的过渡类型",
                normal_color: "普通状态的 Toggle 背景颜色",
                resize_node:
                    "把 Toggle 的 node 的大小重置成 Target 的 node 的大小",
                ...
                zoom_scale:
                    "当用户点击 Toggle 后，Toggle 会缩放到一个值，<br>这个值等于 Toggle 原始 scale * zoomScale, zoomScale 可以为负数",
                auto_gray_effect:
            ...
            shapeModule: {
                ...
                radius: "粒子发射器半径",
                radiusThickness:
                    "粒子发射器发射位置（对 Box 类型的发射器无效）:<br> - 0 表示从表面发射；<br> - 1 表示从中心发射；<br> - 0 ~ 1 之间表示在中心到表面之间发射。",
            ...
            trailSegment: {
                ...
                textureMode:
                    "贴图在拖尾上的展开形式，<br>Stretch 贴图覆盖在整条拖尾上，<br>Repeat 贴图覆盖在一段拖尾上",
            ...
            widget: {
                ...
                align_h_center:
                    "是否对齐 target 水平中点，开启这一选项将取消水平轴上的其他对齐选项",
                align_v_center:
                    "是否对齐 target 垂直中点，开启这一选项将取消垂直轴上的其他对齐选项",
                align_mode:
                    "指定 Widget 的对齐模式，用于决定运行时 Widget 应该何时刷新。",
                top: "本节点顶边和 target 顶边的距离，可输入负值，默认单位为像素（px），也可以输入百分比，根据 target height 和百分比数值计算出距离",
                bottom: "本节点底边和 target 底边的距离，可输入负值，默认单位为像素（px），也可以输入百分比，根据 target height 和百分比数值计算出距离",
                ...
                horizontal_center:
                    "水平居中的偏移值，可输入负值，默认单位为像素（px），也可以是百分比",
                vertical_center:
                    "垂直居中的偏移值，可输入负值，默认单位为像素（px），也可以是百分比",
            },
            layout: {
                layout_type:
                ...
                vertical_direction:
                    "垂直排列子节点的方向，包括：<br> 1. TOP_TO_BOTTOM, 从上到下排列 <br> 2. BOTTOM_TO_TOP, 从下到上排列",
                horizontal_direction:
                    "水平排列子节点的方向，包括：<br> 1. LEFT_TO_RIGHT, 从左到右排列 <br> 2. RIGHT_TO_LEFT, 从右到左排列",
                cell_size: "网格布局中，规定每一个网格的大小",
                start_axis:
                    "网格布局中，子物体排版时的起始方向轴，支持水平和垂直两个方向。",
                constraint:
                    "网格布局中，内容布局约束，包括：<br> 1.NONE，无约束 <br> 2.FIXED_ROW，行数固定 <br> 3.FIXED_COL，列数固定",
                constraint_number: "网格布局中，内容布局约束的行或列数量",
                affected_scale: "子节点缩放比例是否影响布局",
                align_horizontal:
                    "自动对齐。在 Type 为 Horizontal 时自动对齐纵坐标",
                align_vertical:
            ...
            editbox: {
                ...
                editing_ended:
                    "结束编辑文本输入框时触发的事件回调<br>在单行模式下面，一般是在用户按下回车或者点击屏幕输入框以外的地方调用该函数<br>如果是多行输入，一般是在用户点击屏幕输入框以外的地方调用该函数",
                editing_return:
                    "当用户按下回车按键时的事件回调<br>如果是单行输入框，按回车键还会使输入框失去焦点",
            },
            videoplayer: {
                resourceType:
            ...
            UICoordinateTracker: {
                ...
                sync_events:
                    "映射数据事件<br>回调的第一个参数是映射后的本地坐标，第二个是距相机距离",
            },
            subContextView: {
            ...
            dragon_bones: {
                dragon_bones_asset:
                    "骨骼信息数据，拖拽 Dragon Bones 导出的骨骼动画信息 json 资源到这里来开始使用",
                dragon_bones_atlas_asset:
                    "Texture 信息数据，拖拽 Dragon Bones 导出的 Texture 信息 json 资源到这里来开始使用",
                armature_name: "当前的 Armature 名称",
                animation_name: "当前播放的动画名称",
                time_scale: "当前骨骼中所有动画的时间缩放率",
                play_times:
                    "播放默认动画的循环次数<br> - 1 表示使用配置文件中的默认值<br>0 表示无限循环<br> > 0 表示循环次数",
            ...
            particle_system: {
                ...
                spriteFrame: "粒子贴图定义",
                texture:
                    "粒子贴图，只读属性，请使用 sprite Frame 属性来替换贴图",
                ...
                renderer: "粒子渲染模块",
                renderCulling:
                    "是否开启粒子剔除功能。<br>开启该项将会生成一个粒子发射器包围盒，若包围盒不在摄像机的可见范围内，该粒子发射器便会被剔除。<br>粒子发射器被剔除后的行为请参考下面的 Culling Mode。",
                cullingMode:
                    "粒子发射器被剔除之后的行为，可设置的选项包括 Pause, Pause and Catchup, Always Simulate。<br>选择 Pause 时，若粒子发射器包围盒不在摄像机的可见范围内，粒子暂停模拟。若恢复可见，则粒子会接着上次暂停的时间继续模拟；<br>选择 Pause and Catchup 时，若粒子发射器包围盒不在摄像机的可见范围内，粒子暂停模拟。若恢复可见，则粒子会以当前的时间开始模拟；<br>选择 Always Simulate 时，无论粒子发射器包围盒是否在摄像机的可见范围内，粒子都会一直模拟，只是不在摄像机的可见范围内时不进行渲染。",
                alignSpace:
                    "粒子对齐方向空间，可设置的选项包括：视角空间、世界空间和局部空间。<br>选择视角空间时，粒子网格的旋转方向将会跟随摄像机的视角方向；<br>选择世界空间时，粒子网格的方向将会使用发射器节点的世界空间旋转方向；<br>选择局部空间时，粒子网格使用发射器节点的局部空间旋转方向。",
            ...
            mask: {
                ...
                alphaThreshold:
                    "Alpha 阈值，只有当模板的像素的 Alpha 大于 Alpha Threshold 时，才会绘制内容（不支持 Canvas 模式）",
            ...
            block_input_events: {
                brief_help:
                    "该组件将拦截所有输入事件，防止输入穿透到屏幕下方的其它节点，一般用于屏幕上层 UI 的背景。",
            },
            safe_area: {
                symmetric: "是否基于屏幕对称",
            },
            ...
            features: {
                ...
                gfx_webgl2: {
                    label: "WebGL 2.0",
                    description:
                        "包含对 WebGL 2.0 图形 API 的支持。<br>当 WebGL 2.0 在目标平台上不可用时会自动回退至 WebGL 1.0。",
                },
                gfx_webgpu: {
                    label: "WebGPU",
                ...
                sorting_2d: {
                    label: "2D 渲染排序",
                    description:
                        "根据 Sorting2D 组件中的配置对 UIRenderer 进行排序。",
                },
                rich_text: {
                    label: "富文本",
                    description: "富文本支持。",
                ...
                custom_pipeline: {
                    label: "新渲染管线",
                    description:
                        "面向数据的Render Graph渲染管线，可以自由构建跨平台的渲染算法，并针对目标平台优化。",
                },
                custom_pipeline_post_process: {
                    label: "后处理模块（已废弃）",
                    description:
                        "此选项用于兼容旧项目(Custom管线)，新项目请使用 Builtin 自带的后处理流程。",
                },
                legacy_pipeline: {
                    label: "原渲染管线",
                ...
                websocket: {
                    label: "WebSocket",
                    description:
                        "对原生启用 WebSocket。其中iOS/macOS使用SocketRocket实现, Android使用OkHttp实现, Windows使用libwebsockets实现。",
                },
                websocket_server: {
                    label: "WebSocket Server",
                    description:
                        "对原生启用 WebSocket Server。注意: 必须同时启用 WebSocket。",
                },
                flags: {
                    spine: {
                        loadManual: {
            ...
            graphics: {
                lineWidth: "线条宽度",
                lineJoin:
                    "用来设置 2 个长度不为 0 的相连部分（线段、圆弧、曲线）如何连接在一起",
            ...
            physics3d: {
                rigidbody: {
                    ...
                    allowSleep: "是否允许自动休眠",
                    linearDamping:
                        "线性阻尼，用于衰减线性速度，值越大，衰减越快",
                    angularDamping: "角阻尼，用于衰减角速度，值越大，衰减越快",
                    useGravity: "是否使用重力",
                    linearFactor:
                        "线性因子，用于缩放每个轴方向上的物理数值（速度或力）",
                    angularFactor:
                        "角因子，用于缩放每个轴方向上的物理数值（速度或力）",
                },
                collider: {
                    attached: "碰撞体所附加的刚体",
                    ...
                    plane_constant:
                        "在本地坐标系中，平面从原点开始沿着法线运动的距离",
                    mesh_mesh: "所使用的网格资源",
                    mesh_convex:
                        "是否使用凸包近似代替，顶点数应小于255，开启后可以支持动力学",
            ...
            reflection_probe: {
                fastBake:
                    "勾选则不会对生成的 cubemap 进行 GGX 卷积，烘焙速度非常快。",
            },
            hbao: {
                radiusScale:
                    "环境光遮蔽的范围，通过调整该值可以让暗部区域更好的按场景尺度进行适配",
                aoSaturation: "环境光遮蔽的饱和度，该值越大则越暗",
                needBlur:
                    "开启可以获得更柔和的效果，噪点更少，但会耗费一些性能",
            },
            bloom: {
                enableAlphaMask:
                    "透明通道标志位，如果开启此功能，请同时降低物体材质的固有色a通道输出，a 值小于 1 的物体将不会有泛光",
                useHdrIlluminance:
                    "使用场景 HDR 亮度来筛选泛光区域，必须同时开启 HDR 模式和 CC_USE_FLOAT_OUTPUT 宏",
                threshold:
                    "光晕的亮度阈值，大于此亮度的区域才会产生泛光，该值单位是人眼所见的 LDR 亮度，和曝光无关",
                iterations:
                    "模糊迭代次数，该值越大则光晕范围越大、越柔和，但性能也会有所降低",
                intensity: "泛光强度，该值越大则光晕越亮，请适度调节",
            },
            color_grading: {
                contribute:
                    "color grading 的贡献度，可在0-1的范围内调节其对颜色的影响程度",
                originalMap:
                    "支持任意 Nx1 的长条图和 8x8 的方块图，系统自动适配。内置 LUT 贴图路径 internal/dependencies/textures/lut/",
            },
            taa: {
                sampleScale:
                    "TAA 采样范围，推荐使用较小的值，过大的值可能导致抖动",
                feedback:
                    "历史帧混合值，该值越大抗锯齿效果越好，但是画面可能会变糊",
            },
            fsr: {
                sharpness: "画面锐化度",
            },
